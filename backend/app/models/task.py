...
class Task(Base):
    ...
    example_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("source_examples.id"), nullable=False
    )
    ...
    example: Mapped["SourceExample"] = relationship(back_populates="tasks")
    ...
    consensus_results: Mapped[list["ConsensusResult"]] = relationship(
        back_populates="task",
        cascade="all, delete-orphan",
        lazy="selectin",
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ("
            "'created', 'suggested', 'assigned', 'submitted', "
            "'needs_review', 'resolved', 'exported'"
            ")",
...
class Assignment(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('assigned', 'submitted', 'skipped', 'expired')",
