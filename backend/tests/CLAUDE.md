...
| `test_exports.py` | Behavioural tests for the export pipeline: JSONL + CSV file generation with the documented field set, no-eligible-tasks empty file, idempotent re-run (already-exported tasks skipped), `FileResponse` download with `application/x-ndjson` / `text/csv`, list ordering, `label_source='review'` vs `'consensus'` derivation, plus 404 (unknown project / export), 409 (download before completion), and 422 (invalid format) edge cases. Uses `monkeypatch.setattr(settings, 'exports_dir', str(tmp_path))` to isolate file output and `db_session.expire_all()` after POST to read post-BackgroundTask state |
...
| `test_metrics.py` | Behavioral tests for `GET /projects/{project_id}/metrics`: 404 on unknown project, empty-project zero/`null`/`{}` defaults, mixed-status counts summing to `total_tasks` with `exportable_task_count` covering only `resolved` rows (not `exported`), `final_label_distribution` Counter over `relevance` values, `avg_human_agreement` arithmetic mean across latest `ConsensusResult.agreement_score`, `model_human_agreement_rate` excluding tasks without a `ModelSuggestion` from the denominator, and per-project isolation. Drives multi-status seeds directly via `Task.status=...` and full consensus runs via `compute_consensus`; uses the SQLite-backed `client` + `db_session` fixtures from `conftest.py` |
